...
```{r}
write.csv(pollutionAVG,"~/Library/CloudStorage/GoogleDrive-wncheng@ucdavis.edu/我的雲端硬碟/Fall 22/CA-Election-2022-Data-Challenge-master/code/poplution.csv", row.names = FALSE)
```


## CO2_AVG
```{r}
ggplot(pollutionAVG, aes(x=CO2_AVG, color=Fuel)) +
  geom_histogram(alpha=0.2,fill="#999999") 
```

```{r}
ggplot(pollutionAVG, aes(x=CO2_AVG, color=Fuel)) + geom_density(alpha=.2, fill= "#999999" )
```

```{r}
pollutionAVG %>%
  mutate(Fuel = fct_reorder(Fuel, CO2_AVG)) %>% 
  ggplot(aes(x=CO2_AVG, color=Fuel)) + 
  geom_boxplot() + labs(y = "CO2 (million tons per mile)") + coord_flip()
```

#N2O_AVG
```{r}
ggplot(pollutionAVG, aes(x= log(N2O_AVG+0.00000000000000000001), color=Fuel)) +
  geom_histogram(alpha=0.3,fill="#999999")#+geom_freqpoly(alpha=1, fill= "#999999" )
```

```{r}
ggplot(pollutionAVG, aes(x= log(N2O_AVG+0.00000000000000000001), color=Fuel))+
  geom_freqpoly(alpha=1, fill= "#999999" )
```
...
pollutionAVG %>%
  ...
  ggplot(aes(x=log(N2O_AVG+0.000000000000001), color=Fuel)) + 
  geom_boxplot() + labs(y = "N2O (million tons per mile)") + coord_flip()
```
...
## CH4_AVG
```{r}
ggplot(pollutionAVG, aes(x= log(CH4_AVG+0.00000000000000001), color=Fuel)) +
  geom_histogram(fill="white") + geom_density(alpha=.2, fill= "#999999" ) 
```
...
pollutionAVG %>%
  ...
  ggplot(aes(x=log(CH4_AVG+0.00000000000000001), color=Fuel)) + 
  geom_boxplot() + labs(y = "CH4 (million tons per mile)") + coord_flip()
```
```
...
ggplot(pollutionAVG, aes(y = CO2_AVG, x = Fuel)) +
  geom_histogram(stat='identity') + labs(y = "CO2 (million tons per mile)")
```
...
ggplot(data = income_clean) + 
  stat_summary(
    mapping = aes(x = Class, y = Federal_AGI),
    fun.min = min,
    fun.max = max,
    fun = median
  )
...
```
```{r}
ggplot(income_clean, aes(x = factor(Class), y = AGI_class)) +
...
```{r}
library(ggplot2)
ggplot(budget_clean, aes(CO2, Fund)) + 
  geom_point() + 
  geom_line() +
  xlab("Year published") + 
  ylab("Proportion fund against CO2")
```
```{r}
plot(budget_clean$Fund, budget_clean$CO2, main = "Fund and CO2 Relationship", xlab = "Fund",ylab="CO2",pch = 19)
```
...
```{r}
boxplot(budget_clean$CO2)
```


```{r}
budget_clean <- budget_clean[-c(9,21,22),]
plot(budget_clean$Fund, budget_clean$CO2)
...
slope.OBS = lm(CO2 ~ Fund, data = budget_clean)$coefficients[2]
R = 2000
n=19
all.perm.slopes = sapply(1:R,function(i){
  the.data = budget_clean
  the.data$Fund = sample(the.data$Fund,n,replace = FALSE)
  slope.i = lm(CO2 ~ Fund, data = the.data)$coefficients[2]
  return(slope.i)
})
